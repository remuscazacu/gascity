...
func reconcileDrainAckStopPending(
	...
	if err != nil || obs.Running || obs.Alive {
		...
		// the unmutated bead.
		queueDrainAckAsyncStop(cityPath, store, sp, cfg, session.ID, name, session.Metadata["instance_token"], tp.Hints.ProcessNames, asyncStopTracker, stderr)
...
func finalizeDrainAckStopPendingSessions(
	...
	for i := range sessions {
		...
		if err != nil || obs.Running || obs.Alive {
			queueDrainAckAsyncStop(cityPath, store, sp, cfg, session.ID, name, session.Metadata["instance_token"], nil, asyncStopTracker, stderr)
			continue
		}
